...
public:
			...
			if (!q_estado) {
				...
				if (mysql_num_rows(resultado) > 0) {
					...
					if (addp == 'S') {
						...
						string insert = "INSERT INTO `almacen`.`proveedores` (`Proveedor`, `Nit`, `Direccion`, `Telefono`) VALUES ('" + proveedor + "', '" + nit + "', '" + dir + "', '" + t + "');";
	...
	void compraenca(int prov) {
		...
		if (cn.getConectar()) {
			...
			string insertc = "INSERT INTO `almacen`.`compras` (`no_orden_compra`, `idproveedor`, `fecha_orden`, `fechaingreso`) VALUES ('" + ord + "', '" + pr + "', '" + fecha() + "', 'now()');";
				...
				if (!q_estado) {
					...
					if (mysql_num_rows(resultado) > 0) {
						while (fila = mysql_fetch_row(resultado)) {
							...
							string str = "INSERT INTO compras_detalle (idcompras,idproducto,cantidad,precio_costo)VALUES((SELECT MAX(idcompras) FROM compras), `" + prodto + "`, `" + canti + "`, `" + price + "`);";
	...
	void eliminarcompra() {
		...
		if (cn.getConectar()) {
			...
			string eliminard = "DELETE FROM `almacen`.`compras_detalle` WHERE (`idcompra` = (SELECT idcompra FROM compras WHERE no_orden_compra = '" + o + "'));";
			string eliminarv = "DELETE FROM `almacen`.`compras` WHERE (`no_orden_compra ` = '" + o + "');";
